...
public class UserWindow extends JFrame {
	...
	private int activeAuctionId = -1; // This determines which Listing is loaded
										// in the Listing Details Window
	...
	public UserWindow() {
		...
		/*
		 * new CreateListing() //Strategy Pattern to make extensible for future
		 * types of listings new CreateUser() //Will take username and send it
		 * to the server, then the client co new ClientCommunication() //Will
		 * construct itself with username, and bind username to itself so that
		 * the server can call methods defined in the client communication
		 * interface inherited. (Singleton, one get instance) new
		 * PurchaseListings //Could also implement strategy here
		 */

		// ###########################
		// Begin GUI Construction
		setIconImage(Toolkit.getDefaultToolkit()
				.getImage("/Users/carminevalentino/Dropbox/Software/AuctionServerApp/img/icon.png"));
		setResizable(false);
		setTitle("DragonBids");
		...
		tabbedPane.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				...
				case 1: // Listing Detail Tab
					...
					while (it.hasNext()) {
						...
						if (activeAuctionId == thisListing.listingId) {
							...
							long nanosLeft = ChronoUnit.NANOS.between(LocalDateTime.now(),
									thisListing.auctionCompletionDateTime);
							...
							// Populate Listing Details Window with this
							// ListingSkeleton
						}
					}
					break;
				...
				case 3: // Browse Tab
					try {
						listingsList.removeAllElements(); // Clear the Table Out
															// Before
															// Re-populating
		...
		btnLogin.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				/*
				 * Try to create the user with the server If user exists server
				 * will set the local boolean to true Otherwise, the remote
				 * method will return true, and user will be created
				 */
		...
		list.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (2 == e.getClickCount()) {
					...
					tabbedPane.setSelectedIndex(1); // Select Listing Detail
													// Pane
				}
			}
		});
			...
			public void keyPressed(KeyEvent e) {
				if (e.getKeyCode() == KeyEvent.VK_ENTER && "" != userToContact) {
					...
					try {
						...
						stub.notifyClient(
								activeUser + "<" + System.currentTimeMillis() + ">" + ": " + MsgMessage.getText());
	...
	private void connectToServer() {
		while (waitingForServer) {
			try {
				registry = LocateRegistry.getRegistry(); // Works only on
															// localhosts,
															// meaning we have
															// to run the server
															// from the same
															// machine/subnet
				stub = (DragonBidsServer_I) registry.lookup("DragonBids"); // This
																			// is
																			// a
																			// Key
																			// to
	...
	private void createAuction() // Methods which invoke the remote objects need
									// to throw a Remote Exception, and calls sh
	{
		// TODO: This would check to ensure that we have made a connect, and
		// have a bound object
		if (!waitingForServer) {
			...
			try {
				...
				listing.listingTypes = ListingSkeleton.ListingTypes.AUCTION; // The
																				// Client-side
																				// would
																				// handle
																				// determining
	...
	private void modifyListing() throws RemoteException {
		if (!waitingForServer) {
			try {
				...
				// modListing.extendAuctionMinutes = [] // consider changing
				// this to "proposedEndTime"
